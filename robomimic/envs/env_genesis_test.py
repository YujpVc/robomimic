...
class GenesisEnvWrapper(EB.EnvBase):
    def __init__(self, env_name=None, env_config=None, camera_width=84, camera_height=84, **kwargs):
        ...
        self.shelf = self.scene.add_entity(
            gs.morphs.MJCF(
                file="/home/yujp/Genesis/my_models/wooden_two_layer_shelf/wooden_two_layer_shelf.xml",
                pos=(0.0, 0.75, 0.0),
                scale=1.0,
            )
        )
    ...
    def reset(self):
        ...
        # 使用显式变量设置位置
        self.cube.set_pos(np.array([cube_x, cube_y, cube_z]))
        self.cube.set_quat(np.array([1, 0, 0, 0]))
        self.target.set_pos(np.array([0.5, 0.0, 0.01]))
        self.target.set_quat(np.array([1.0, 0.0, 0.0, 0.0]))

        # 重置架子位置（如果需要）
        self.shelf.set_pos(np.array([0.0, 0.75, 0.0]))
    ...
    def get_observation(self):
        ...
        obs = {
            ...
            "object": np.concatenate([
                self.cube.get_pos().cpu().numpy().squeeze().copy(),
                self.cube.get_quat().cpu().numpy().squeeze().copy()
            ]),
        }
    ...
    def get_state(self):
        ...
        cube_pos = self.cube.get_pos().cpu().numpy().squeeze()
        cube_quat = self.cube.get_quat().cpu().numpy().squeeze()

        robot_qvel = self.robot.get_dofs_velocity().cpu().numpy()
        cube_vel = self.cube.get_vel().cpu().numpy().squeeze()
        cube_ang = self.cube.get_ang().cpu().numpy().squeeze()
